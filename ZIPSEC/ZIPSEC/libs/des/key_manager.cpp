...
uint Key_manager::get_new_key(uint original,int cycle){
	...
	if(shifts_count == 1){
		exceeding_bits = (uint)Permutations::get_single_bit(original,28,1);
	}else{
		exceeding_bits = (uint)Permutations::get_single_bit(original,28,1) << 1;
		exceeding_bits |=(uint) Permutations::get_single_bit(original,28,2);
	}
