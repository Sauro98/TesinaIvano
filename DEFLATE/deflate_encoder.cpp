...
Deflate_encoder::Deflate_encoder(std::string to_compress){
	...
	#endif
	static_tree = construct_static_leteral_and_lengths_tree();
	#if DEBUG > 0
		println("END");
	#endif
}
...
long* Deflate_encoder::construct_static_leteral_and_lengths_tree(){
	#if DEBUG > 1
...
int Deflate_encoder::get_static_literal_length(int literal){
...
long Deflate_encoder::get_mask_for_length(int length){
	switch(length){
		case 1:
			return MASK_1_BIT;
		case 2:
