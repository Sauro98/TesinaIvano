...
class Deflate_decoder
{
	public:
		...
		int get_next_static_value(long*current_char,int* current_bit);
		unsigned int read_bits(long* current_char,int* current_bit,int bits_count);
		unsigned int read_bits(long* current_char,int* current_bit,int bits_count,bool reversed);
